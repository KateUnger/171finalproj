...
def handle_prepare(network_server, src_node, dst_node, incoming_seq_num, incoming_pid, incoming_op_num):
    ...
    if int(incoming_seq_num) > ballot_number[0]: # to do: > or >=
        ...
        for temp_ballot_num, temp_op in temp_queue.itemss():
            network_server.send(f"{dst_node} {src_node} NEWOP {temp_ballot_num[0]} {temp_ballot_num[1]} {temp_ballot_num[2]} {temp_op[0]}{' break '}".encode('utf-8'))
            # print(f"\nSEND {dst_node} {src_node} NEWOP {temp_ballot_num[0]} {temp_ballot_num[1]} {temp_ballot_num[2]} {temp_op[0]}")
...
def handle_promise(incoming_op_log, incoming_seq_num, incoming_pid, incoming_op_num):
    ...
    if incoming_op_log:
        ...
        elif int(incoming_seq_num) == ballot_number[0] and int(incoming_pid) >= ballot_number[1]: # to do: might change
...
def select_best_answer(network_server, context_id, query_src, response, incoming_seq_num, incoming_pid, incoming_op_num): 
    ...
    if leader != "P1":
        network_server.send(f"P1 {leader} ANSWER {incoming_seq_num} {incoming_pid} {incoming_op_num} {context_id} {response}{' break '}".encode('utf-8'))
        # print(f"\nSEND P1 {leader} ANSWER {incoming_seq_num} {incoming_pid} {incoming_op_num} {context_id} {response}")
    elif leader == "P1":
        contexts[context_id] += f"\n\nResponse: {response}"
        ...
        network_server.send(f"P1 P2 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}{' break '}".encode('utf-8'))
        network_server.send(f"P1 P3 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}{' break '}".encode('utf-8'))
        # print(f"\nSEND P1 P2 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
        # print(f"\nSEND P1 P3 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
...
def handle_leader_queue(network_server):
        ...
        while True:
            ...
            network_server.send(f"P1 P2 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}{' break '}".encode('utf-8'))
            network_server.send(f"P1 P3 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}{' break '}".encode('utf-8'))
            print(f"\nSEND P1 P2 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
            print(f"\nSEND P1 P3 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
            ...
            while True:
                if accepted_count_map[this_ballot_num] >= 1:
                    ...
                    print(f"\nSEND P1 P2 DECIDE {strip_ballot_num(ballot_number)} {operation}")
                    print(f"\nSEND P1 P3 DECIDE {strip_ballot_num(ballot_number)} {operation}")
...
def start_election(network_server):
    ...
    print(f"\nSEND P1 P2 PREPARE {strip_ballot_num(ballot_number)}")
    print(f"\nSEND P1 P3 PREPARE {strip_ballot_num(ballot_number)}")
            ...
            for message in messages:
                ...
                if consensus_op == "FWD": # FWD {orig_src_node} {leader} {fwd_consensus_op} {ballot_num} {operation}
                    ...
                    if(final_dest != "P1"):
                        ...
                        network_server.send(f"P1 {final_dest} FWD {spliced_op}{' break '}".encode('utf-8'))
                        continue
                    else:
                        spliced_op = spliced_op.replace(f"{src_node} {dst_node} FWD ", "")
                ...
                if consensus_op == "PREPARE": # PREPARE {ballot_num} --> e.g. PREPARE 1 1 2
                    ...
                    if timed_out_op == "NEWOP":
                        ...
                        if dst_node == leader:
                            ...
                            if leader == "P3":
                                network_server.send(f"P1 P2 FWD P1 P3 {timed_out_op} {incoming_seq_num} {incoming_pid} {incoming_op_num} {spliced_op}{' break '}".encode('utf-8'))
                            elif leader == "P2":
                                network_server.send(f"P1 P3 FWD P1 P2 {timed_out_op} {incoming_seq_num} {incoming_pid} {incoming_op_num} {spliced_op}{' break '}".encode('utf-8'))
                        elif dst_node != leader and leader != "P1":
                            if timed_out_op == "NEWOP":
                                spliced_op = spliced_op.replace(f"{src_node} {dst_node} TIMEOUT NEWOP {incoming_seq_num} {incoming_pid} {incoming_op_num} ", "")
...
def handle_user_input(s1, network_server):
    ...
    while True:
        ...
        if leader == "P1":
            with lock:
                leader_queue.append([operation, "P1"])
                semaphore.release()
            ...
            print(f"\nSEND P1 {leader} NEWOP {strip_ballot_num(ballot_number)} {operation}")
            temp_queue[ballot_number] = [operation, "P1"]
        elif leader == "":
