...
def handle_prepare(network_server, src_node, dst_node, incoming_seq_num, incoming_pid, incoming_op_num):
    ...
    if int(incoming_seq_num) > ballot_number[0]: # to do: > or >=
        ...
        print("HERE")
        network_server.send(f"{dst_node} {src_node} LEADER{' break '}".encode('utf-8'))
...
def select_best_answer(network_server, context_id, query_src, response, incoming_seq_num, incoming_pid, incoming_op_num): 
    ...
    if leader != "P1":
        ...
        print(f"\nSENT:\n P1 {leader} ANSWER {incoming_seq_num} {incoming_pid} {incoming_op_num} {context_id} {response}")
    elif leader == "P1":
        contexts[context_id] += f"\n\nResponse: {response}"
        ...
        print(f"\nSENT:\n P1 P2 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
        print(f"\nSENT:\n P1 P3 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
...
def handle_leader_queue(network_server):
        ...
        while True:
            ...
            print(f"\nSENT:\n P1 P2 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
            print(f"\nSENT:\n P1 P3 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
    ...
    while True:
        try:
            ...
            for message in messages:
                ...
                elif consensus_op == "RECOVERY_RESP": # RECOVERY 
                    spliced_op = spliced_op.replace(f"{src_node} {dst_node} RECOVERY_RESP ", "")
                    op_log = ast.literal_eval(spliced_op)
                    
                elif consensus_op == "RECOVERY_ASK": # RECOVERY 
                    network_server.send(f"{dst_node} {src_node} RECOVERY_RESP {op_log}{' break '}".encode('utf-8'))
...
def handle_user_input(s1, network_server):
    while True:
        ...
        if leader == "P1":
            ...
            print(f"\nSENT:\n P1 {leader} NEWOP {strip_ballot_num(ballot_number)} {operation}")
