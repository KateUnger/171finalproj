...
def select_best_answer(network_server, context_id, query_src, response, incoming_seq_num, incoming_pid, incoming_op_num): 
    ...
    if leader != "P2":
        ...
        # print(f"\nSEND P2 {leader} ANSWER {incoming_seq_num} {incoming_pid} {incoming_op_num} {context_id} {response}")
    elif leader == "P2":
        contexts[context_id] += f"\n\nResponse: {response}"
        ...
        # print(f"\nSEND P2 P1 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
        # print(f"\nSEND P2 P3 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
...
def handle_leader_queue(network_server):
        ...
        while True:
            ...
            print(f"\nSEND P2 P1 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
            print(f"\nSEND P2 P3 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
            ...
            while True:
                if accepted_count_map[this_ballot_num] >= 1:
                    ...
                    print(f"\nSEND P2 P1 DECIDE {strip_ballot_num(ballot_number)} {operation}")
                    print(f"\nSEND P2 P3 DECIDE {strip_ballot_num(ballot_number)} {operation}")
...
def start_election(network_server):
    ...
    print(f"\nSEND P2 P1 PREPARE {strip_ballot_num(ballot_number)}")
    print(f"\nSEND P2 P3 PREPARE {strip_ballot_num(ballot_number)}")
...
def handle_user_input(s1, network_server):
    ...
    while True:
        ...
        if leader == "P2":
            ...
            print(f"\nSEND P2 {leader} NEWOP {strip_ballot_num(ballot_number)} {operation}")
