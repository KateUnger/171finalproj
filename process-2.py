...
def handle_leader_queue(network_server):
    while True:
        ...
        while True:
            leader_queue.pop(0)
            network_server.send(f"P2 P1 ACCEPT {strip_ballot_num(ballot_number)}{' break '}".encode('utf-8'))
            network_server.send(f"P2 P3 ACCEPT {strip_ballot_num(ballot_number)}{' break '}".encode('utf-8'))
