...
def select_best_answer(network_server, context_id, query_src, response, incoming_seq_num, incoming_pid, incoming_op_num): 
    ...
    if leader != "P3":
        ...
        print(f"\nSENT:\n P3 {leader} ANSWER {incoming_seq_num} {incoming_pid} {incoming_op_num} {context_id} {response}")
    elif leader == "P3":
        contexts[context_id] += f"\n\nResponse: {response}"
        ...
        print(f"\nSENT:\n P3 P1 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
        print(f"\nSENT:\n P3 P2 RESPONSES {incoming_seq_num} {incoming_pid} {incoming_op_num} {query_src} {context_id} {response_choices}")
...
def handle_leader_queue(network_server):
        ...
        while True:
            ...
            print(f"\nSENT:\n P3 P1 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
            print(f"\nSENT:\n P3 P2 ACCEPT {int(this_ballot_num[0])} {int(this_ballot_num[1])} {int(this_ballot_num[2])} {operation}")
